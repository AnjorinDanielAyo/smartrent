...
function showPanel(dash, panel) {
  document.querySelectorAll('#page-' + dash + ' .dash-panel').forEach(p => p.classList.remove('active'));
  document.querySelectorAll('#page-' + dash + ' .sidebar-link').forEach(l => l.classList.remove('active'));

  const panelEl = document.getElementById(dash + '-' + panel);
  if (panelEl) panelEl.classList.add('active');

  document.querySelectorAll('#page-' + dash + ' .sidebar-link').forEach(function(link) {
    if (link.getAttribute('onclick') && link.getAttribute('onclick').includes("'" + panel + "'")) {
      link.classList.add('active');
    }
  });

  const titles = {
    'browse': 'Browse items', 'my-rentals': 'My rentals', 'history': 'Rental history',
    'overview': 'Overview', 'my-listings': 'My listings', 'requests': 'Rental requests',
    'add-listing': 'Add new listing', 'profile': 'Profile',
  };
...
function selectRole(r) {
  selectedRole = r;
  document.getElementById('role-renter').classList.toggle('selected', r === 'renter');
  document.getElementById('role-rentee').classList.toggle('selected', r === 'rentee');
}
...
function initRenterDash() {
  ...
  if (!u) return;
  document.getElementById('renter-name-sidebar').textContent  = u.fname + ' ' + u.lname;
...
async function renderBrowse() {
  ...
  grid.innerHTML = '<div style="color:var(--text3);padding:40px;text-align:center"><i class="lucide-loader" style="font-size:24px"></i></div>';
  ...
  try {
    ...
    grid.innerHTML = items.map(function(item) {
      const avBadge = item.status === 'available'
    ...
    grid.innerHTML = '<div class="empty-state"><i class="lucide-wifi-off"></i><h3>Could not load items</h3><p>' + e.message + '</p></div>';
  }
}

function filterItems() { renderBrowse(); }
...
async function renderMyRentals() {
  ...
  try {
    ...
    const reqs = data.requests.filter(r => ['pending','approved'].includes(r.status));
    if (reqs.length === 0) {
      el.innerHTML = '<div class="empty-state"><i class="lucide-calendar-x"></i><h3>No active rentals</h3><p>Your approved and pending rental requests will appear here.</p><button class="btn btn-primary btn-sm" onclick="showPanel(\'renter\',\'browse\')"><i class="lucide-search" style="font-size:14px"></i> Browse items</button></div>';
      return;
    }
...
async function renderHistory() {
  ...
  try {
    const data = await api('/api/my-requests');
    const hist = data.requests.filter(r => ['completed','cancelled','declined'].includes(r.status));
    if (hist.length === 0) {
      el.innerHTML = '<div class="empty-state"><i class="lucide-clock"></i><h3>No rental history yet</h3><p>Completed and cancelled rentals will show up here.</p></div>';
      return;
    }
...
function initRenteeDash() {
  const u = state.currentUser;
  if (!u) return;
  document.getElementById('rentee-name-sidebar').textContent  = u.fname + ' ' + u.lname;
...
async function renderOwnerOverview() {
  ...
  document.getElementById('rentee-welcome').textContent = 'Welcome back, ' + u.fname;
  try {
    const [listData, reqData] = await Promise.all([api('/api/my-listings'), api('/api/incoming-requests')]);
    const listings = listData.listings;
    const reqs     = reqData.requests;
    const pending  = reqs.filter(r => r.status === 'pending').length;
    const active   = reqs.filter(r => r.status === 'approved').length;
    const earnings = listings.filter(l => l.status === 'rented').reduce((s,l) => s + l.price * 7, 0);

    document.getElementById('stat-listings').textContent      = listings.length;
    document.getElementById('stat-pending').textContent       = pending;
    document.getElementById('stat-pending-label').innerHTML   = pending > 0 ? '<span class="change-up">Needs your attention</span>' : '<span class="text-muted">No new requests</span>';
    document.getElementById('stat-active').textContent        = active;
    document.getElementById('stat-earnings').textContent      = '₦' + earnings.toLocaleString();
    document.getElementById('stat-earnings-label').textContent = earnings > 0 ? 'From active rentals' : 'Start listing to earn';
...
async function renderMyListings() {
  const el = document.getElementById('my-listings-grid'); if (!el) return;
  try {
    const data     = await api('/api/my-listings');
    const listings = data.listings;
    if (listings.length === 0) {
      el.innerHTML = '<div class="empty-state"><i class="lucide-box"></i><h3>No listings yet</h3><p>You haven\'t listed any items. Add your first listing to start earning.</p><button class="btn btn-primary btn-sm" onclick="showPanel(\'rentee\',\'add-listing\')"><i class="lucide-plus" style="font-size:14px"></i> Add listing</button></div>';
      return;
    }
...
async function renderRequests() {
  const el = document.getElementById('requests-content'); if (!el) return;
  try {
    const data = await api('/api/incoming-requests');
    const reqs = data.requests;
    if (reqs.length === 0) {
      el.innerHTML = '<div class="empty-state"><i class="lucide-inbox"></i><h3>No requests yet</h3><p>Rental requests from other users will appear here once your items are listed.</p></div>';
      return;
    }
    const badgeMap = { pending:'badge-warning', approved:'badge-success', declined:'badge-danger' };
    const rows = reqs.map(function(r) {
...
function renderProfile(dash) {
  const u   = state.currentUser; if (!u) return;
  const el  = document.getElementById(dash + '-profile-content'); if (!el) return;
...
function showToast(msg, type) {
  ...
  icon.className  = type === 'success' ? 'lucide-check-circle' : 'lucide-alert-circle';
